...
class ModTools:
    ...
    async def on_member_update(self, before, after):
        ...
        specialRoles = BOT_CONFIG.get("specialRoles", {})
        
        if specialRoles.get('bots') is None:
            return
            
        botRole = discord.utils.get(after.roles, id=int(specialRoles.get('bots')))
            
        if (botRole is not None) and (botRole not in before.roles) and (not before.bot):
            await after.remove_roles(botRole, reason="User is not an authorized bot.")
