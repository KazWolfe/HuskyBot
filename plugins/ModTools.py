...
class ModTools(commands.Cog):
        ...
        if not is_role_mentionable:
            if target >= ctx.me.top_role:
                await ctx.send(embed=discord.Embed(
                    ...
                    description="I may not trigger a roleping for this role, as it is higher than my own top role.",
                    color=Colors.ERROR
                ))
                return
    ...
    async def set_nickname(self, ctx: commands.Context, member: discord.Member,
        ...
        if member == ctx.bot.user:
            await ctx.send(embed=discord.Embed(
                title="Moderator Toolkit",
                description="**You can not change the bot's nickname.**",
                color=Colors.DANGER
            ))
            return
        ...
        if member.top_role.position >= ctx.message.author.top_role.position:
            await ctx.send(embed=discord.Embed(
                title="Moderator Toolkit",
                description=f"User `{member}` can not be altered, as they are not below you in the role hierarchy.",
                color=Colors.DANGER
            ))
            return

        await member.edit(nick=nickname, reason=f"Nick edit requested by {ctx.author}")

        nick_notif = "cleared" if nickname is None else f"set to `{nickname}`"

        await ctx.send(embed=discord.Embed(
            title=Emojis.NAMETAG + f" Nickname updated!",
            description=f"{member}'s nickname has been {nick_notif}!",
            color=Colors.SUCCESS
        ))
    ...
    async def mass_ban(self, ctx: commands.Context, reason: str, *users):
        ...
        for user_selector in users:
            try:
                user = await converter.convert(ctx, user_selector)

                if user == ctx.author:
                    raise discord.DiscordException("IS_SELF")

                if user == ctx.bot.user:
                    raise discord.DiscordException("IS_BOT")
                ...
                report['failed'].append(f"{user_selector} - {''.join(e.args) if e.args else 'OTHER_ERR'}")
                LOG.error("Massban Error", e)
                continue

        embed = discord.Embed(
            title="Mass Ban Report",
            description=f"{len(report['succeeded'])} users banned.\n"
            f"{len(report['failed'])} failed to ban. Nonexistent user or other error.",
            color=Colors.INFO
        )
